/**
 * @file check_deque.cpp
 * @brief Deque type inference for the Bishop type checker.
 */
...
/**
 * Infers the type of a deque creation expression.
 */
TypeInfo check_deque_create(TypeCheckerState& state, const DequeCreate& deque) {
    (void)state;
    return {"Deque<" + deque.element_type + ">", false, false};
}

/**
 * Type checks a method call on a deque.
 */
TypeInfo check_deque_method(TypeCheckerState& state, const MethodCall& mcall, const string& element_type) {
    auto method_info = bishop::get_deque_method_info(mcall.method_name);

    if (!method_info) {
        error(state, "Deque has no method '" + mcall.method_name + "'", mcall.line);
