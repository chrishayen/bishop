/**
 * @file check_stack.cpp
 * @brief Stack type inference for the Bishop type checker.
 */
...
/**
 * Infers the type of a stack creation expression.
 */
TypeInfo check_stack_create(TypeCheckerState& state, const StackCreate& stack) {
    (void)state;
    return {"Stack<" + stack.element_type + ">", false, false};
}

/**
 * Type checks a method call on a stack.
 */
TypeInfo check_stack_method(TypeCheckerState& state, const MethodCall& mcall, const string& element_type) {
    auto method_info = bishop::get_stack_method_info(mcall.method_name);

    if (!method_info) {
        error(state, "Stack has no method '" + mcall.method_name + "'", mcall.line);
